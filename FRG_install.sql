...
as
  select DT,db_id,table_id,index_id,partition,atype,fragment_count,frag_pct,page_used_pct,page_count,frag_size_pages 
    from (
      select DT,db_id,table_id,index_id,partition,atype,fragment_count,frag_pct,page_used_pct,page_count,frag_size_pages
...
select F.DT,Dbname,SchemaName,TableName,IndexName,S.Partition,IndexType,rows,TotalSpaceMb,
   ...
   page_used_pct,isnull(frag_size_pages,0) as frag_size_pages
